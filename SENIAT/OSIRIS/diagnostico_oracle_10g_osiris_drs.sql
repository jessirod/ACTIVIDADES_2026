-- =============================================================================
-- DIAGNOSTICO ORACLE 10g - Checklist inicial de salud / baseline
-- Ruta: ORACLE/DIAGNOSTIC/diagnostico_oracle_10g.sql
-- Compatible: 10gR2 (10.2.0.x) - enfoque conservador
-- Puede correr en 10gR1 con algunas secciones vacias/error si no aplica ASM/FRA/DG
-- NO usa: VERSION_FULL, ORACLE_MAINTAINED, DBA_REGISTRY_SQLPATCH,
--         DBA_TAB/IND_STATISTICS.STALE_STATS, NULLS LAST, GAP_STATUS
-- Ejecutar como SYSDBA o usuario con privilegios DBA_* / V$ / ASM
-- =============================================================================
-- Uso:
--   sqlplus / as sysdba
--   SPOOL diagnostico_10g_<host>.txt
--   @diagnostico_oracle_10g.sql
--   SPOOL OFF
-- =============================================================================

SPOOL diagnostico_10g_osiris_drs.txt
...
PROMPT
PROMPT ######################################################################
PROMPT  DIAGNOSTICO ORACLE 10g - INICIO
...
SELECT i.version AS version_instance,
       (SELECT p.version
         ...
         WHERE p.product LIKE 'Oracle Database%'
           AND ROWNUM = 1) AS version_product,
       CASE
...
PROMPT
PROMPT Nota 10g: confirmar PSU/CPU con opatch en OS (scripts .sh).
PROMPT En BD se usa REGISTRY$HISTORY / PRODUCT_COMPONENT_VERSION.

-- =============================================================================
-- 2. NIVEL DE PARCHE (10g)
-- =============================================================================
PROMPT
PROMPT ######################################################################
PROMPT  2. NIVEL DE PARCHE (10g)
PROMPT ######################################################################

PROMPT
PROMPT 2a. Historial de registro (REGISTRY$HISTORY) - ultimos 30
...
PROMPT
PROMPT 2b. PRODUCT_COMPONENT_VERSION
SELECT product,
       version,
       status
...
SELECT i.table_owner AS owner,
 ...
 WHERE i.table_owner NOT IN (
         'SYS','SYSTEM','OUTLN','DIP','ORACLE_OCM','DBSNMP','WMSYS',
         'EXFSYS','CTXSYS','XDB','ANONYMOUS','ORDSYS','ORDDATA','ORDPLUGINS',
         'MDSYS','OLAPSYS','SI_INFORMTN_SCHEMA','SYSMAN','MGMT_VIEW',
         'TSMSYS','WKSYS','WKPROXY','WK_TEST','DMSYS','PERFSTAT'
       )
...
SELECT owner,
       ...
       ROUND(SUM(NVL(space,0)) * 8 / 1024, 2) AS mb_aprox
  FROM dba_recyclebin
 GROUP BY owner
...
SELECT i.owner,
 ...
 ORDER BY NVL(CASE WHEN NVL(i.num_rows, 0) = 0 THEN NULL
                   ELSE ROUND(i.clustering_factor / NULLIF(i.num_rows, 0), 2)
              END, 0) DESC,
          size_mb DESC;
...
SELECT df.tablespace_name,
 ...
 ORDER BY NVL(ROUND((df.bytes_alloc - NVL(fs.bytes_free, 0)) * 100.0
             / NULLIF(df.bytes_alloc, 0), 2), 0) DESC,
          df.tablespace_name;
...
-- =============================================================================
-- 14. OBJETOS EN TABLESPACE USERS
-- =============================================================================
PROMPT
PROMPT ######################################################################
...
-- 10g: usa DBA_TABLES / DBA_INDEXES (sin STALE_STATS)
-- =============================================================================
PROMPT
PROMPT ######################################################################
PROMPT  15. ESTADISTICAS DESACTUALIZADAS (> 7 DIAS) - via DBA_TABLES/INDEXES
PROMPT ######################################################################
...
SELECT owner,
 ...
 ORDER BY NVL(last_analyzed, TO_DATE('19000101','YYYYMMDD')), owner, table_name;
...
SELECT owner,
 ...
 WHERE owner NOT IN (
   ...
   AND (last_analyzed IS NULL OR last_analyzed < SYSDATE - 7)
 ORDER BY NVL(last_analyzed, TO_DATE('19000101','YYYYMMDD')), owner, index_name;

PROMPT
PROMPT 15c. Resumen estadisticas viejas / sin stats
...
PROMPT
PROMPT 19a. Parametros db_recovery*
...
, CASE WHEN SPACE_LIMIT = 0 THEN 'N/A'
       WHEN ((SPACE_Used/SPACE_LIMIT)*100 < 80) THEN 'NORMAL'
       WHEN ((SPACE_Used/SPACE_LIMIT)*100 BETWEEN 80 and 90) THEN 'WARNING'
       ELSE 'CRITICAL' END "Status"
FROM V$RECOVERY_FILE_DEST;

-- =============================================================================
-- 20. PARAMETROS CRITICOS
-- =============================================================================
PROMPT
PROMPT ######################################################################
...
PROMPT
PROMPT 20b. Otros parametros importantes
SELECT name,
       value,
       isdefault,
       ismodified
...
PROMPT
PROMPT 20c. log_archive_dest_* configurados (no vacios)
SELECT name,
       value,
       isdefault,
       ismodified
...
PROMPT
PROMPT 21e. Standby redo logs (V$STANDBY_LOG) - si aplica
SELECT group#,
       thread#,
       sequence#,
       ROUND(bytes/1024/1024, 2) AS size_mb,
...
PROMPT
PROMPT 22.1 Modo de operacion actual
SELECT open_mode,
       log_mode
  FROM v$database;
...
PROMPT
PROMPT 22.7 Destinos de archive (V$ARCHIVE_DEST_STATUS) - columnas basicas 10g
SELECT dest_id,
       status,
       database_mode,
       recovery_mode,
...
PROMPT
PROMPT ######################################################################
PROMPT  DIAGNOSTICO ORACLE 10g - FIN
PROMPT ######################################################################
