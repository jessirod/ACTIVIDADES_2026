...
alter database add logfile thread 1 group 101 ('+REDO01_RENTA','+REDO02_RENTA') size 2048M; 
alter database add logfile thread 1 group 102 ('+REDO01_RENTA','+REDO02_RENTA') size 2048M; 
alter database add logfile thread 1 group 103 ('+REDO01_RENTA','+REDO02_RENTA') size 2048M; 
alter database add logfile thread 1 group 104 ('+REDO01_RENTA','+REDO02_RENTA') size 2048M;


--alter database add logfile thread 2 group 201 ('+REDO01_RENTA','+REDO02_RENTA') size 2048M; 
--alter database add logfile thread 2 group 202 ('+REDO01_RENTA','+REDO02_RENTA') size 2048M; 
--alter database add logfile thread 2 group 203 ('+REDO01_RENTA','+REDO02_RENTA') size 2048M; 
--alter database add logfile thread 2 group 204 ('+REDO01_RENTA','+REDO02_RENTA') size 2048M; 
--
--
...
**CUANDO ES SINGLE SOLO SE AGREGA UN GRUPO.
...
--alter database drop logfile group 304;
...
alter system set log_archive_dest_1='LOCATION=USE_DB_RECOVERY_FILE_DEST valid_for=(ALL_LOGFILES,ALL_ROLES) MAX_FAILURE=1 REOPEN=5 DB_UNIQUE_NAME=RENTAP ALTERNATE=LOG_ARCHIVE_DEST_10';

alter system set log_archive_dest_10='location=+DATA_RENTA valid_for=(ALL_LOGFILES,ALL_ROLES) DB_UNIQUE_NAME=RENTAP ALTERNATE=LOG_ARCHIVE_DEST_1';

alter system set log_archive_dest_state_1='ENABLE';
alter system set log_archive_dest_state_10='ALTERNATE';
alter system set db_create_online_log_dest_1='+REDO01_RENTA';
alter system set db_create_online_log_dest_2='+REDO02_RENTA';
...
CONFIGURE SNAPSHOT CONTROLFILE NAME TO '+FRA/snapcf_TRZS.f';
...
CONFIGURE CONTROLFILE AUTOBACKUP ON;
CONFIGURE SNAPSHOT CONTROLFILE NAME TO '+FRA_RENTA/snapcf_RENTAP.f';
...
CREATE TABLESPACE DATA_AUDITORIA
DATAFILE 
  '+DATA_RENTA' SIZE 2G AUTOEXTEND OFF
LOGGING
DEFAULT 
  NO INMEMORY
ONLINE
...
grid       87347       1  0 Jan06 ?        00:00:04 /oracle/app/oracle/product/19c/db1/bin/tnslsnr RENTAP -no_crs_notify -inherit
grid      329197       1  0 14:54 ?        00:00:00 /oracle/app/product/19c/grid/bin/tnslsnr LISTENER -no_crs_notify -inherit

-- el listener estaba con el home de oracle - Abrir NETCA en oracle y borrar el listener RENTAP - Abrir netca con grid y modificar el puerto. 
